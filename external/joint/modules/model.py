...
class Model(Encoder):
    ...
    def forward(self, *args, **kwargs):
        ...
        ## labeling_segment loss calculation
        cls_logit = self.label_classifier(encoder_emb)
        cls_pred = P.argmax(cls_logit, axis=-1)
        ...
        ## linking loss calculation
        mask = link_mask * self.kv_mask_gen(cls_pred)
        ...
        return {
            'labeling_segment':
                {
                    'logit': selected_cls_logit,
                    'label': selected_cls_label,
                    'pred': selected_cls_pred,
                    'logit_prim': cls_logit,
                    'label_prim': cls_label,
                    'pred_prim': cls_pred,
                    'mask': cls_mask
                }
            ,
